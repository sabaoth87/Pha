...
public class PHA_SudokuGame extends Activity {
	
	private static final String TAG = "PHA:LANX(Su)";
	public static final String KEY_DIFFICULTY = "tk.prototype.phalanxv1.difficulty";
	...
	private PHA_PuzzleView puzzleView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		DIFFICULTY_EASY);
		puzzleGrid = getPuzzle(diff);
		calculateUsedTiles();
		puzzleView = new PHA_PuzzleView(this);
	...
	protected void showKeypadOrError(int x, int y){
		...
		if (tiles.length == 9){
			Toast toast = Toast.makeText(this,
					"No More Moves", Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
		} else {
			Log.d(TAG, "showKeypad: used=" + toPuzzleString(tiles));
			Dialog v = new SudokuKeypad(this, tiles, puzzleView);
			v.show();
		}
	}
	...
	private int getTile(int x, int y){
		return puzzleGrid[y * 9 + x];
	}
	/** Change the tile at the given coordinates */
	private void setTile(int x, int y, int value){
		puzzleGrid[y * 9 + x] = value;
	}
